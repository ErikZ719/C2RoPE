...
from llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, IMG_TOKEN_LEN
import torch
...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len=None, img_token_pos=None):
...
class FCALlamaRotaryEmbedding(LlamaRotaryEmbedding):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        super().__init__(dim=dim, max_position_embeddings=max_position_embeddings, base=base, device=device)
        self.dim = dim
        self.max_position_embeddings = max_position_embeddings
        self.base = base
        inv_freq = 1.0 / (self.base ** (torch.arange(0, self.dim, 2).float().to(device) / self.dim))
        ...
        # Build here to make `torch.jit.trace` work.
        self._set_fca_cos_sin_cache(
            seq_len=max_position_embeddings, device=self.inv_freq.device, dtype=torch.get_default_dtype(), img_token_pos=0
        )

    def _set_fca_cos_sin_cache(self, seq_len, device, dtype, img_token_pos):
        self.max_seq_len_cached = seq_len
        t = torch.arange(self.max_seq_len_cached, device=device, dtype=self.inv_freq.dtype)
        t_pre = t[:img_token_pos]
        t_img = t[img_token_pos:img_token_pos+576]
        t_last = t[img_token_pos+576:]
        
        freqs_pre = torch.outer(t_pre, self.inv_freq)
        ...
        y_list = []
        t_list = []
        raster_matrix = torch.arange(0, size*size, device=device, dtype=torch.long).reshape(size, size)
        for j in range(size):
            for i in range(size):
                ...
                if j == half - 1 or j == half:
                    ...
                    y_val = -(j - half)
                x_list.append(x_val + 11)
                y_list.append(y_val + 11)
                t_list.append((raster_matrix[j, i] + img_token_pos).item())
        t_tensor = torch.tensor(t_list, device=device, dtype=self.inv_freq.dtype)
        x_tensor = torch.tensor(x_list, device=device, dtype=self.inv_freq.dtype)
        y_tensor = torch.tensor(y_list, device=device, dtype=self.inv_freq.dtype)

        freqs_img_t = torch.outer(t_tensor, self.inv_freq[:58])
        freqs_img_x = torch.outer(x_tensor, self.inv_freq[58:61])
        freqs_img_y = torch.outer(y_tensor, self.inv_freq[61:])
        freqs_img = torch.cat([freqs_img_t, freqs_img_x, freqs_img_y], dim=1)
        # print("okok")
        
        freqs_last = torch.outer(t_last, self.inv_freq)
        freqs = torch.cat([freqs_pre, freqs_img, freqs_last], dim=0)
        # freqs = torch.outer(t, self.inv_freq)
        emb = torch.cat((freqs, freqs), dim=-1)
        self.register_buffer("cos_cached", emb.cos().to(dtype), persistent=False)
        self.register_buffer("sin_cached", emb.sin().to(dtype), persistent=False)
    ...
    def forward(self, x, seq_len=None, img_token_pos=None):
        ...
        if img_token_pos is None:
            raise ValueError("img_token_pos 必须传入")
        if seq_len > self.max_seq_len_cached:
            self._set_fca_cos_sin_cache(seq_len=seq_len, device=x.device, dtype=x.dtype, img_token_pos=img_token_pos)
...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            self.fca_rotary_emb = FCALlamaRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                base=self.rope_theta,
            )
    ...
    def forward(
        ...
        batch_img_token_pos=None,  # where visual tokens start
        ...
        if batch_img_token_pos is not None:
            for img_token_pos in batch_img_token_pos:
                if img_token_pos == -1:
                    ...
                    if not hasattr(self, "fca_rotary_emb"):
                        self.fca_rotary_emb = FCALlamaRotaryEmbedding(
                            self.head_dim,
                            max_position_embeddings=self.max_position_embeddings,
                            base=self.rope_theta,
                        )
                    cos, sin = self.fca_rotary_emb(x=value_states, seq_len=kv_seq_len, img_token_pos=img_token_pos)
                    # print("okok")
                    query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)
        else:
            cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
...
class FCALlamaSdpaAttention(LlamaAttention):
    """
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # Ensure fca_rotary_emb is always available for this class
        self.fca_rotary_emb = FCALlamaRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                base=self.rope_theta,
...
class FCALlamaModel(LlamaPreTrainedModel):
    """
        ...
        for img_token_pos in batch_img_token_pos:
            ...
            # image input.
                ...
                if seq_len == 1:
                    ...
                    if img_token_pos > seq_len - IMG_TOKEN_LEN:
                        cca_position_ids = torch.arange(seq_len).to(torch.long).to(self.device)
                    else:
            ...
            for (b_idx, img_token_pos), cca_position_ids in zip(enumerate(batch_img_token_pos), batch_cca_position_ids):
                if img_token_pos == -1:
                    ...
                    for pos in torch.arange(img_token_pos, img_token_pos + H // 2):
                        ...
                        if cca_attention_mask is not None:
...
class FCALlamaForCausalLM(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = FCALlamaModel(config)
        self.vocab_size = config.vocab_size
